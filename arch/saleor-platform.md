...
- `docker-compose.yml` — the only orchestration file; defines all services, volumes, network.
...
- `replica_user.sql` — Postgres init script; creates read-only `saleor_read_only` user.
...
- `README.md`, `LICENSE`, `.gitignore` (ignores only `.idea`).

## Services (docker-compose.yml)
All on one bridge network `saleor-backend-tier` except `dashboard` (no network declared — uses default).
...
## Wiring
- `api` `depends_on`: `db`, `cache`, `jaeger`. `worker` `depends_on`: `cache`, `mailpit`. (Note: `worker` does not declare a `db` dependency though it needs the DB — inferred ordering relies on `db` being up.)
...
- `api` and `worker` both load `common.env` + `backend.env` (identical config, so worker hits same DB/cache/broker).
- `dashboard` talks to `api` only from the browser via `localhost:9000` → API at `localhost:8000` (api sets `DASHBOARD_URL=http://localhost:9000/`); they are not wired server-side and dashboard isn't on the backend network.
- Tracing: api/worker export OTLP to `jaeger:4317` (`OTEL_*` in backend.env).

## Volumes
Named: `saleor-db` (Postgres data, local driver), `saleor-cache` (Valkey data, local driver), `saleor-media` (shared api/worker media, no explicit driver). Bind: `./replica_user.sql` → Postgres `docker-entrypoint-initdb.d` (`:ro,z`). `jaeger` `/tmp` is tmpfs.
...
- First-run setup is manual (README): `migrate`, then `populatedb --createsuperuser` (admin `admin@example.com` / `admin`).
- **Workspace note:** upstream API port here is `8000`; the overlay remaps it to `8001` via `dev/docker-compose.dev.yml` (per `CLAUDE.md`). Do not edit this file to change dev behavior — change the overlay.

## CI/CD & tests
- Single GitHub Actions workflow `.github/workflows/test-platform.yml`, trigger `pull_request`: `docker compose build` then `docker compose run api pytest -n logical --allow-hosts cache` (runs the core image's pytest suite, parallel via pytest-xdist).
- No deploy pipeline — repo is dev-only, ships nothing.

## Security & compliance notes
- All credentials are hardcoded dev defaults committed to git: `SECRET_KEY=changeme`, Postgres `saleor/saleor`, read-only replica user `saleor_read_only/saleor`, admin `admin@example.com/admin`. Safe only because this is local-dev; **never deploy as-is**.
- Every backend service publishes its port to the host (5432, 6379, 8000, etc.) — open on the dev machine, no auth on cache. Fine on localhost, risky on shared/exposed hosts.
- `HTTP_IP_FILTER_ENABLED=True` with loopback allowed — README notes loopback-allow should be `False` and filter behavior differs for production.
- Pinned image tags: api/dashboard `3.23`, postgres `15-alpine`, valkey `8.1-alpine`; `jaeger`, `mailpit`, `axllent/mailpit` are unpinned (`latest`), so builds are not fully reproducible (inferred).
- `worker` runs with `-B` (embedded beat) — fine for single-node dev, would double-schedule if scaled to multiple workers.
- `setup-e2e-db.sh` uses BSD `sed -i ''` syntax (macOS-only) and expects an `update-automation-snapshot.sql` not present in the repo; maintainer/E2E tooling, not part of normal flow.
